...
{
    header("location:displayrawmaterial.php");
}
$q = mysqli_query($conn, "SELECT * from raw_material where raw_material_id='{$editid}'") or die(mysqli_error($conn));
$qrow = mysqli_fetch_array($q);
//print_r($qrow);
$msg = "";
if ($_POST) 
{
    $rawmaterial_id = mysqli_real_escape_string($conn, $_POST['rawmaterial_id']);
    $raw_name = mysqli_real_escape_string($conn, $_POST['raw_name']);
    
    $raw_qoh = mysqli_real_escape_string($conn, $_POST['raw_qoh']);
    

    $qupdate = mysqli_query($conn, "update raw_material set raw_material_name='{$raw_name}',QOH='{$raw_qoh}' where raw_material_id='{$rawmaterial_id}'") or die(mysqli_error($conn));
    if ($qupdate) {

        $msg = '<div class="alert alert-success mt-4" role="alert">
                     Your Raw Material editted successfully!!!
                </div>';
    }
}
?>
                    ...
                    <div class="container-fluid">
                        <div class="row">
                            ...
                            <div class="outer-w3-agile col-xl mt-3 mr-xl-3">
                                <br>
                                <h2 class="tittle-w3-agileits mt-4 mb-3 text-center fs-1" style="color:#1F45FC">Edit Raw
                                    Material
                                    Information
                                </h2>
                                <br>
                                <form method="post" enctype="multipart/form-data" data-parsley-validate=""
                                    ...
                                    <input type="hidden" class="form-control" id="inputEmail3"
                                        value="<?php echo $qrow['raw_material_id'] ?>" name="rawmaterial_id">

                                    <form method="post" enctype="multipart/form-data" data-parsley-validate=""
                                        data-parsley-priority-enable=true
                                        data-parsley-required="This value is required">
                                        <div class="form-group row">
                                            <label for="inputPassword3" class="col-sm-2 col-form-label"> Rawmaterial
                                                Name
                                            </label>
                                            <div class="col-sm-10">
                                                <input type="text" class="form-control" id="name" name="raw_name"
                                                    value="<?php echo $qrow['raw_material_name'] ?>"
                                                    placeholder="Enter raw_matrial Name" required autocomplete="off" data-parsley-trigger="keyup"
                                data-parsley-required-message="Raw Material is required"
                                data-parsley-pattern="[A-Za-z]+"
                                        ...
                                        <br>
                                        <div class="form-group row">
                                            <label for="inputPassword3" class="col-sm-2 col-form-label"> Rawmaterial QOh
                                            </label>
                                            <div class="col-sm-10">
                                                <input type="text" class="form-control" id="inputPassword3"
                                                    name="raw_qoh" value="<?php echo $qrow['QOH'] ?>"
                                                    placeholder="Enter Product QOH" required
                                                    data-parsley-required-message="Qoh is required"
                                        ...
                                        <div class="form-group row">
                                            <div class="col-sm-10">
                                                ...
                                                <button style="margin-left:2%" type="button"
                                                    onclick="window.location='displayrawmaterial.php'"
