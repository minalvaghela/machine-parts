...
$date = "";
$inquiry_status = "";
//$sql1 ="SELECT * from inquiry where user_user_id='$_SESSION[user_name]'";
            ...
            <div id="content">
                <section class="tables-section">
                    <div class="outer-w3-agile mt-3">
                        <br>
                        <h2 class="tittle-w3-agileits mt-4 mb-3 text-center fs-1" style="color:#1F45FC">View Inquiry
                        </h2>
                        <br>
                        <table class="table center">
                            ...
                            <tbody>
                                ...
                                <?php
                                $q = mysqli_query($conn, "SELECT * from inquiry") or die(mysqli_error($conn));
                                while ($inquiryrow = mysqli_fetch_array($q)) {
                                    echo "<tr>";
                                    $sid=$inquiryrow['services_services_id'];
                                    $get_name2=mysqli_fetch_row(mysqli_query($conn,"select name from services where services_id='$sid'"))[0];
                                    echo "<td>{$inquiryrow['inquiry_id']}</td>";
                                    echo "<td>{$inquiryrow['user_user_id']}</td>";
                                    echo "<td>{$inquiryrow['discripation']}</td>";
                                    echo "<td>{$inquiryrow['date']}</td>";
                                    echo "<td>{$inquiryrow['inquiry_status']}</td>";
                                    $get_name = mysqli_fetch_row(mysqli_query($conn,"SELECT `product_name` FROM `product` WHERE `product_id` = '". $inquiryrow['product_product_id']."'"))[0];
                                    echo "<td>$get_name</td>";
                                    echo "<td>$get_name2</td>";
                                    if($inquiryrow['inquiry_status'] == 'init')
                                    {
                                        echo "<td><a href='Inquiry_Answer.php?id={$inquiryrow['user_user_id']} &i_id={$inquiryrow['inquiry_id']}'> <input type='button' name='ans' value='Answer' class='btn btn-primary' style='margin:2px'></a></td>";
                                    }
                                    else
                                    {
                                        echo "<td><input type='button' name='ans' value='Answer' class='btn btn-primary' style='margin:2px' disabled></td>";
