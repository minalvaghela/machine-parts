        ...
        <div class="wrapper">
            ...
            <?php
              $msg="";
                                if (isset($_GET['delid'])) {
                                    $deleteid = $_GET['delid'];
                                    $q = mysqli_query($conn, "delete from purchase_return_detail where purchase_return_purchasereturn_id= '{$deleteid}'") or die(mysqli_error($conn));
            ...
            <div id="content">
                <section class="tables-section">
                    <div class="outer-w3-agile mt-3">
                        <br>
                        <h2 class="tittle-w3-agileits mt-4 mb-3 text-center fs-1" style="color:#1F45FC">Purchase _return
                            Information</h2>
                        <table class="table">
                          <form method="POST" enctype="multipart/form-data" name="supp" action="getprucheid.php">
                                    ...
                                    <div class="col-sm-5">
                                        <select name="p1" required="">
                                          ...
                                          while($row = mysqli_fetch_assoc($result)){
                                             echo "<option value='".$row['purchase_id']."'>".$row['purchase_id']."</option>";
                                          }
                                    ?>
                                        </select>
                                    </div>
                                </div>
                                <button type="submit" name="btn" class="btn btn-primary" style="margin-left: 80%">
                                    <b>Generate purchase returnOrder</b>
                                </button>
                            ...
                            <thead class="thead-dark">
                                <tr>
                                    ...
                                    <th scope="col">Purchase_return _id </th>
                            ...
                            <tbody>
                                ...
                                <?php
                                $q = mysqli_query($conn, "SELECT *from purchase_return order by purchasereturn_id desc") or die(mysqli_error($conn));
                                
                               
                                while ($productrow = mysqli_fetch_array($q)) {
                                    echo "<tr>";
                                   // $pid=$productrow['purchase_purchase_id'];
                                   // echo $pid;
                                    echo "<td>{$productrow['purchasereturn_id']}</td>";
                                    echo "<td>{$productrow['date']}</td>";
                                    echo "<td><a href='viewpurchase_return.php?pid={$productrow['purchasereturn_id']}'> <input type='button' name='view' value='View' class='btn btn-primary' style='margin:2px'></a>| <a href='purchase_return.php?delid={$productrow['purchasereturn_id']}'><input type='button' value='delete' class='btn btn-danger' style='margin:2px'></a></td>";
                                    echo "</tr>";
                                }
