...
{
    header("location:viewpurchase_return.php");
}
$q = mysqli_query($conn, "SELECT * from purchase_return_detail where purchase_return_purchasereturn_id='{$editid}'") or die(mysqli_error($conn));                               

while ($row = mysqli_fetch_array($q)) 
{
     //$id=$row['raw_material_raw_material_id'];
     $qty=$row['qty'];   
...
if (isset($_POST['update'])) 
{
    ...
    $id1=$_GET['editid'];
    $rowid=$_GET['rowid'];
    //$qupdate = mysqli_query($conn, "UPDATE `purchase_return_detail` SET qty='{$qty2}' where raw_material_raw_material_id='{$rowid}' && purchase_return_purchasereturn_id='{$editid}'") or die(mysqli_error($conn));
    $qupdate = mysqli_query($conn, "UPDATE `purchase_return_detail` SET `qty`='{$qty2}' WHERE 
    `purchase_return_purchasereturn_id`='{$editid}' AND `raw_material_raw_material_id`='{$rowid}'");
    //echo $qupdate;
    if ($qupdate) 
    {
                                        ...
                                        <div class="form-group row">
                                            <div class="col-sm-10">
                                                ...
                                                <button style="margin-left:2%" type="button"
                                                    onclick="window.location='viewpurchase_return.php?pid=<?php echo $editid?>'" class="btn btn-info">
                                                    View </button>
                                            </div>
                                        </div>
                                    </form>
