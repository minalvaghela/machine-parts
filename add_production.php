...
/*if ($insert_purdetail) {
    $msg = '<div class="alert alert-success mt-4" role="alert">
               Your Production Added Successfully!!!
          </div>';
}*/
?>
<!DOCTYPE html>
                        ...
                        <div class="row">
                            <div class="outer-w3-agile col-xl mt-3 mr-xl-3">
                                ...
                                <input type="hidden" value="<?php echo $proid;?>" name="id">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-10">
                                            <input type="number" class="form-control" id="qty" name="qty"
                                                placeholder="Enter Qty.." required autocomplete="off"
                                                data-parsley-trigger="keyup"
                                                data-parsley-required-message="Product Qty is required"
                                    ...
                                    <br>
                                    <div class="form-group row">
                                        <label class="col-sm-2 col-form-label">RAWMATERIAL</label>
                                        <div class="col-sm-10">
                                            <select name="raw_id" class="form-control" id="raw_id" required=""  data-parsley-required-message="Select any Raw Material">
                                    ...
                                    <div class="form-group row">
                                        <div class="col-sm-10">
                                            <button type="submit" name="add" class="btn btn-primary"> Add Production</a>
                                            </button>
                                            <button style="margin-left:2%" type="button"
                                                onclick="window.location='displayproduction.php'" class="btn btn-info">
                                                View
                                            </button>
                                        </div>
                                    </div>
                            ...
                            <tbody>
                                <?php
                                $q = mysqli_query($conn, "SELECT *from production_detail where production_production_id='$proid'") or die(mysqli_error($conn));
    
                               
                                while ($productrow = mysqli_fetch_array($q)) {
                                    echo "<tr>";
                                    $id=$productrow['raw_material_raw_material_id'];
                                    $rawname=mysqli_query($conn,"select raw_material_name from raw_material where raw_material_id='$id'");
                                    $name=mysqli_fetch_row($rawname)[0];
                                    $id2=$productrow['production_production_id'];
                                    $proname=mysqli_query($conn,"select product_product_id from production where production_id='$id2'");
                                    $name2=mysqli_fetch_row($proname)[0];
                                    $pname=mysqli_query($conn,"select product_name from product where product_id='$name2'");
                                    $name3=mysqli_fetch_row($pname)[0];

                                    echo "<td>{$productrow['production_production_id']}</td>";
                                    echo "<td>$name</td>";
                                    echo "<td>{$productrow['rawmaterial_oty']}</td>";
                                    echo "<td>$name3</td>";
                                    echo "</tr>";
                                }
