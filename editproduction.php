...
$q = mysqli_query($conn, "SELECT *from production_detail where raw_material_raw_material_id='{$editid}'") or die(mysqli_error($conn));                               
while ($row = mysqli_fetch_array($q)) 
{
     $id=$row['production_production_id'];
     $qty=$row['rawmaterial_oty'];
    // $price=$row['price'];    
...
if (isset($_POST['update'])) 
{
    ...
    $id1=$_GET['editid'];
    $rowid=$_GET['rowid'];
    $qupdate = mysqli_query($conn, "UPDATE `production_detail` SET rawmaterial_oty='{$qty2}' WHERE  raw_material_raw_material_id='{$rowid}'") or die(mysqli_error($conn));
    //echo $qupdate;
    if ($qupdate) 
    {
$msg = '<div class="alert alert-success mt-4" role="alert">
Your production editted successfully!!!
        </div>';  
    }
}
?>
                                        ...
                                        <div class="form-group row">
                                            <div class="col-sm-10">
                                                ...
                                                <button style="margin-left:2%" type="button"
                                                    onclick="window.location='viewproduction.php?pid=<?php echo $id?>'" class="btn btn-info">
                                                    View </button>
                                            </div>
                                        </div>
                                    </form>
