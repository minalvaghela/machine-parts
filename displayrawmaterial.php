    ...
    <section class="home-section">
        ...
        ?>
            <?php
                                ...
                                if (isset($_GET['delid'])) {
                                    $deleteid = $_GET['delid'];
                                    $q = mysqli_query($conn, "delete from raw_material_detail where raw_material_raw_material_id = '{$deleteid}'") or die(mysqli_error($conn));
                                    
                                    $q = mysqli_query($conn, "delete from raw_material where raw_material_id = '{$deleteid}'") or die(mysqli_error($conn));
                                    if ($q)
                                     {
            ...
            <div id="content">
                <section class="tables-section">
                    <div class="outer-w3-agile mt-3">
                        <br>
                        <h2 class="tittle-w3-agileits mt-4 mb-3 text-center fs-1" style="color:#1F45FC">Raw Material
                            Information
                        </h2>
                        <br>
                        <table class="table">
                            <thead class="thead-dark">
                                <tr>
                                    ...
                                    <th scope="col"> Rawmaterial Qoh </th>
                            ...
                            <tbody>
                                ...
                                <?php
                                $q = mysqli_query($conn, "SELECT *from raw_material") or die(mysqli_error($conn));
                                
                               
                                while ($productrow = mysqli_fetch_array($q)) {
                                    echo "<tr>";
                                    
                                    echo "<td>{$productrow['raw_material_name']}</td>";
                                    echo "<td>{$productrow['QOH']}</td>";
                                    echo "<td> <a href='editraw_material.php?editid={$productrow['raw_material_id']}'><input type='button' value='Edit' class='btn btn-primary' style='margin:2px'></a> | <a href='displayrawmaterial.php?delid={$productrow['raw_material_id']}]'><input type='button' value='delete' class='btn btn-danger' style='margin:2px'></a></td>";
                                    echo "</tr>";
                                }
                                ?>
                            </tbody>
                        ...
                        <h5 class="tittle-w3-agileits mb-4">
                            <a href="add_rawmaterial.php">
                                <button type="button" class="btn btn-primary" style="margin-left: 78%">
                                    <b>Add rawmaterial Details</b>
                                </button>
                            </a>
                        </h5>
                    </div>
